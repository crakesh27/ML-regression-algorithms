...
def gradient_descent(x, y, theta, lamb, alpha, n):
	...
	while(i<n):
		...
		theta = theta - (alpha * (((1/y.size) * np.dot(x.T, error)) + lamb*(theta)))
		iterations.append(i)
		losses.append(loss)
		w1.append(theta[1])
		w2.append(theta[2])
...
def stochiastic_gradient_descent(x, y, theta, lamb, alpha, n):
	...
	while(i<n):
		...
		for j in range(len(prediction)):
			...
			theta = theta - (alpha * (np.dot(x[j], error) + lamb*(theta)))
		iterations.append(i)
		losses.append(loss)
		w1.append(theta[1])
		w2.append(theta[2])
...
loss = loss_test_data(X_train, Y_train, theta)
print("Ridge Regression - Stochiastic Gradient Descent")
print("Weights : ", end='')
print(theta)
print("Loss on test data : ", end='')
print(round(loss,3))
